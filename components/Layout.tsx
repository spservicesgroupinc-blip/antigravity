...
export const Layout: React.FC<LayoutProps> = ({
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-slate-50 font-sans md:pb-0 relative">

      {/* Windows Titlebar Drag Region - Overlay */}
      {isInstalled && (
        <div className="fixed top-0 left-0 right-0 h-8 z-50 w-full titlebar-drag-region bg-transparent pointer-events-auto" />
      )}

      {/* Toast Notification */}
      {notification && (
        <div className={`fixed top-4 right-4 md:top-8 md:right-8 z-[70] flex items-center gap-3 px-6 py-4 rounded-2xl shadow-2xl border animate-in slide-in-from-top-5 duration-300 ${notification.type === 'success'
      ...
      <aside className="hidden md:flex flex-col w-64 bg-white border-r border-slate-200 h-screen sticky top-0 z-20">
        <div className="p-6 border-b border-slate-100 relative z-[60]">
          <div className="cursor-pointer titlebar-no-drag inline-block" onClick={() => setView('dashboard')}>
            <RFESmallLogo />
          </div>
        </div>
        ...
        <nav className="flex-1 p-4 overflow-y-auto">
          ...
          {!isInstalled && (
            installPrompt ? (
              ...
              <div className="mt-4 px-4 py-2 bg-slate-50 rounded-xl text-xs text-slate-400 border border-slate-100">
                <p className="font-bold mb-1 uppercase tracking-wider text-[10px]">Install App:</p>
                <p>Chrome: Click ⋮ menu → "Install Spray Foam Pro..."</p>
              </div>
            )
          )}
        </nav>
      ...
      {/* Main Content */}
      <main className="flex-1 min-w-0 md:p-8 p-4 overflow-x-hidden pt-8 md:pt-8">
        {/* Mobile Header */}
        <div className="md:hidden flex items-center justify-between mb-6 relative z-[60]">
          <div className="flex items-center gap-2 titlebar-no-drag cursor-pointer" onClick={() => setView('dashboard')}>
