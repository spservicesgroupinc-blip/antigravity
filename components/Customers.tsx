...
import { sendWelcomeEmail, sendGenericEmail } from '../services/emailApi';
...
export const Customers: React.FC<CustomersProps> = ({
    ...
    const handleOpenEmailModal = (customer: CustomerProfile) => {
        if (!customer.email) return alert("No email address listed for this customer.");
        setEmailSubject('');
        setEmailBody('');
    ...
    const handleSendEmail = async () => {
        ...
        try {
            const result = await sendGenericEmail(customer.email, emailSubject, emailBody);
            ...
            if (result.status === 'success') {
                ...
                const logEntry: CommunicationLogEntry = {
                    ...
                    date: new Date().toISOString(),
                    type: 'Email',
                    content: `Sent Email: "${emailSubject}"`
                };
                ...
                alert("Failed to send email. Status: " + result.status);
            }
        } catch (e) {
            console.error(e);
                                ...
                                <div className="flex flex-col gap-3">
                                    ...
                                    {customer.email && (
                                        <button
                                            ...
                                            className="w-full bg-sky-600 hover:bg-sky-700 text-white p-4 rounded-2xl font-black text-sm uppercase tracking-widest flex items-center justify-center gap-3 shadow-lg shadow-sky-200 transition-all active:scale-95"
                                        >
                            ...
                            {sortedLogs.length === 0 ? (
                                ...
                                sortedLogs.map((log) => (
                                    <div key={log.id} className="flex gap-4 group">
                                        <div className={`mt-1 flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center border-2 ${log.type === 'Call' ? 'bg-emerald-50 border-emerald-100 text-emerald-600' :
                                        ...
                                        <div className="flex-1">
                                            <div className="flex justify-between items-center mb-1">
                                                <span className={`text-[10px] font-black uppercase tracking-widest ${log.type === 'Call' ? 'text-emerald-600' : 'text-slate-500'
                                                    }`}>{log.type}</span>
                ...
                {emailModalOpen && (
                    <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                        <div className="bg-white rounded-3xl w-full max-w-lg p-8 shadow-2xl animate-in fade-in zoom-in duration-300">
                            <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight mb-2 flex items-center gap-2">
                                <Mail className="w-5 h-5 text-brand" /> Send Email
                            </h3>
                            <p className="text-sm text-slate-500 mb-6 font-medium">To: {state.customers.find(c => c.id === viewingCustomerId)?.email}</p>
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <input
                                        type="text"
                                        className="w-full border-2 border-slate-100 rounded-2xl p-4 font-bold text-slate-900 focus:ring-2 focus:ring-brand outline-none"
                                ...
                                <div>
                                    <label className="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-1 tracking-widest">Message</label>
                                    <textarea
                                        className="w-full border-2 border-slate-100 rounded-2xl p-4 font-medium text-slate-700 min-h-[150px] resize-none focus:ring-2 focus:ring-brand outline-none"
                            ...
                            <div className="flex gap-3 pt-6">
                                <button onClick={() => setEmailModalOpen(false)} disabled={isSendingEmail} className="flex-1 p-4 border-2 border-slate-100 rounded-2xl font-black uppercase text-xs tracking-widest text-slate-400 hover:bg-slate-50 transition-colors">Cancel</button>
                                <button onClick={handleSendEmail} disabled={isSendingEmail || !emailSubject || !emailBody} className="flex-1 p-4 bg-slate-900 text-white rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-slate-800 shadow-lg shadow-slate-200 transition-all flex items-center justify-center gap-2">
                                    {isSendingEmail ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Send className="w-4 h-4" /> Send Message</>}
                                </button>
                            </div>
                        </div>
                    </div>
