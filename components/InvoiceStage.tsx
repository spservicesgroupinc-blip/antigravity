...
export const InvoiceStage: React.FC<InvoiceStageProps> = ({
    ...
    const [processingAction, setProcessingAction] = useState<'save' | 'pay' | 'email' | null>(null);
    ...
    const handleEmailInvoice = async () => {
        // Determine correct email and name to use. 
        // 1. Look up the customer in the active 'state.customers' list using the ID. 
        //    This ensures we get the latest profile updates (like a newly added email) 
        //    even if the estimate record snapshot is stale.
        const customerId = currentRecord?.customerId || currentRecord?.customer?.id;
        const dbCustomer = state.customers.find(c => c.id === customerId);

        // 2. Fallback to record snapshot, then global profile (mostly for new drafts)
        const targetEmail = dbCustomer?.email || currentRecord?.customer?.email || state.customerProfile.email;
        const targetName = dbCustomer?.name || currentRecord?.customer?.name || state.customerProfile.name;

        if (!targetEmail) {
            alert(`Customer (${targetName}) does not have an email address saved. Please update the customer profile.`);
            return;
        }
        ...
        try {
            const { buildDocumentPDF } = await import('../utils/pdfGenerator');

            // Generate Invoice PDF
            const { doc, filename } = buildDocumentPDF(
                state, // Uses current editor state
                results,
                'INVOICE',
                currentRecord // Pass current record for meta info if needed
            );
            ...
            const response = await sendDocumentEmail({
                to: targetEmail,
                subject: `Invoice #${invoiceNum} from RFE Foam App`,
                body: `Hello ${targetName},\n\nPlease find attached Invoice #${invoiceNum} dated ${state.invoiceDate}.\n\nTotal Due: $${results.totalCost.toLocaleString()}\n\nThank you for your business.`,
            ...
            if (response.status === 'success') {
                ...
                alert("Failed to email invoice: " + response.message);
            }
                ...
                <div className="p-4 md:p-6 flex flex-col md:flex-row justify-between items-center gap-6">
                    ...
                    <div className="flex items-center gap-4 w-full md:w-auto">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-2 mt-1">
                                <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-widest ${isPaid ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'
                                    }`}>
                                    Status: {statusLabel}
                                </span>
                            </div>
                    ...
                    <div className="flex flex-col md:flex-row gap-3 w-full md:w-auto">
                        <button
                            ...
                            className="px-6 py-4 bg-sky-50 border border-sky-100 hover:bg-sky-100 text-sky-700 rounded-xl font-bold uppercase text-xs tracking-widest flex items-center justify-center gap-2 transition-all w-full md:w-auto"
                        >
                            {processingAction === 'email' ? <Loader2 className="w-4 h-4 animate-spin" /> : <Mail className="w-4 h-4" />}
                            Email
                        </button>
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    {actuals ? (
                        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                            ...
                            {/* COMPLETION PHOTOS */}
                            {actuals.completionPhotos && actuals.completionPhotos.length > 0 && (
                                <div className="mt-6">
                                    <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3">
                                        <CheckCircle2 className="w-3 h-3" /> Job Photos ({actuals.completionPhotos.length})
                                    </div>
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                                        {actuals.completionPhotos.map((url: string, i: number) => (
                                            <div key={i} className="aspect-square rounded-lg overflow-hidden bg-slate-100 border border-slate-200 relative group">
                                                <img src={url} className="w-full h-full object-cover" alt="Job Proof" />
                                                <a href={url} target="_blank" rel="noreferrer" className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 flex items-center justify-center text-white text-xs font-bold transition-opacity">View</a>
                                            </div>
                                        ))}
                                    </div>
                                </div>
