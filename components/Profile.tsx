                            ...
                            <input type="file" accept="image/*" onChange={async (e) => {
                                ...
                                if (f) {
                                    try {
                                        ...
                                        // 2. Upload to Drive (Cloud Storage)
                                        // We need session info to get spreadsheetId/folderId
                                        const sessionStr = localStorage.getItem('foamProSession');
                                        if (sessionStr) {
                                            const session = JSON.parse(sessionStr);
                                            // Optimistic Update (Show local preview while uploading)
                                            onUpdateProfile('logoUrl', compressed);

                                            // Perform Upload
                                            const { uploadImage } = await import('../services/api');
                                            const url = await uploadImage(compressed, `CompanyLogo_${new Date().getTime()}.jpg`, session.spreadsheetId, session.folderId);

                                            if (url) {
                                                onUpdateProfile('logoUrl', url);
                                            }
                                        } else {
                                            // Fallback for Trial/Offline - just local
                                            onUpdateProfile('logoUrl', compressed);
                                        }
                                    } catch (err) {
                                        console.error(err);
