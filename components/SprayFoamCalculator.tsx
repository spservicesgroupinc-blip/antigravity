...
// Capture install prompt early to avoid React hydration race conditions
...
const SprayFoamCalculator: React.FC = () => {
    ...
    useEffect(() => {
        // If prompt was captured before mount, set it
        ...
        const handler = (e: any) => {
            ...
            capturedPrompt = e;
            setDeferredPrompt(e);
            console.log('[PWA] Install prompt captured in useEffect');
        };
    ...
    // Allow creating new warehouse items from estimate screen
    ...
    if (session.role === 'crew') {
        return (
            <CrewDashboard
                ...
                onSync={forceRefresh} // Pass FORCE REFRESH (Sync Down) instead of manual sync (Push)
                installPrompt={deferredPrompt}
                onInstall={handleInstallApp}
            />
        );
        ...
        >
            {ui.view === 'dashboard' && (
                <Dashboard
                    ...
                    onEditEstimate={(rec) => {
                        ...
                        // Direct Workflow: If Job is Completed (Review Needed), go straight to Invoice/Review Stage
                        if (rec.status === 'Work Order' && rec.executionStatus === 'Completed') {
                            loadEstimateForEditing(rec, 'invoice_stage');
                        } else {
                            // Otherwise go to standard detail view
                            loadEstimateForEditing(rec, 'estimate_detail');
                        }
                    }}
            ...
            {ui.view === 'estimate_detail' && ui.editingEstimateId && (
                <EstimateDetail
                    ...
                    results={results} // Calculated from hydrated state
