...
import { sendAccountCreationEmail } from '../services/emailApi';
    ...
    try {
      if (activeTab === 'crew') {
        ...
        if (!isSignup) {
            ...
            // Perform Signup
            // The backend returns the session + the generated Crew PIN in the 'data' field logic
            const responseData: any = await signupUser(formData.username, formData.password, formData.companyName, formData.email);
            
            // If we get data back, it means success
            if (responseData) {
                // Send Welcome Email with credentials
                // Note: We use responseData.crewPin because the backend generates it
                try {
                    ...
                    await sendAccountCreationEmail(
                        ...
                        formData.username,
                        formData.password,
                        responseData.crewPin || "Check Admin Profile"
                    );
                } catch(emailErr) {
                ...
                setError("Username taken or signup failed.");
            }
        }
      }
    } catch (err: any) {
