...
import { DollarSign, HardHat, Receipt, Filter, Plus, Trash, CheckCircle2, AlertCircle, Clock, TrendingUp, TrendingDown, Wallet, PieChart, Fuel, ArrowRight, AlertTriangle, BarChart3, RefreshCw, Activity } from 'lucide-react';
...
export const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  // --- AUTO REFRESH POLLER ---
  // Polls every 15 seconds to check for "In Progress" or "Completed" updates from the crew
  useEffect(() => {
                                        ...
                                        <tr key={est.id} className="hover:bg-slate-50 transition-colors cursor-pointer" onClick={() => onEditEstimate(est)}>
                                            ...
                                            <td className="px-6 py-4">
                                                ...
                                                {est.status === 'Work Order' && est.executionStatus === 'In Progress' ? (
                                                    <span className="flex items-center gap-1 bg-red-100 text-red-700 px-2 py-1 rounded-full text-[10px] font-black uppercase tracking-tighter w-fit animate-pulse border border-red-200">
                                                        <Activity className="w-3 h-3" /> Live: Spraying
                                                    </span>
                                                ) : est.status === 'Work Order' && est.executionStatus === 'Completed' ? (
                                                    <span className="flex items-center gap-1 bg-emerald-100 text-emerald-700 px-2 py-1 rounded-full text-[10px] font-black uppercase tracking-tighter w-fit animate-pulse border border-emerald-200 shadow-sm shadow-emerald-100">
