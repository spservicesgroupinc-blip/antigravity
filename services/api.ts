...
const apiRequest = async (payload: any, retries = 2): Promise<ApiResponse> => {
    ...
    // Inject Token if available in Session
    try {
...
/**
 * Signals that a crew has started a job
 */
...
export const uploadImage = async (base64Data: string, fileName: string, spreadsheetId: string, folderId?: string): Promise<string | null> => {
    const result = await apiRequest({ action: 'UPLOAD_IMAGE', payload: { base64Data, fileName, spreadsheetId, folderId } }, 0); // No retry for heavy uploads
    if (result.status === 'success') return result.data.url;
...
export const signupUser = async (username: string, password: string, companyName: string, email: string): Promise<UserSession | null> => {
    const result = await apiRequest({ action: 'SIGNUP', payload: { username, password, companyName, email } });
    if (result.status === 'success') return result.data;
