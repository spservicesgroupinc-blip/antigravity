...
export const buildDocumentPDF = (
  ...
  // Use record customer/settings if available, otherwise use state (for preview/unsaved)
  ...
  // Inventory: use record snapshot if available, otherwise state
  ...
  } else if (type === 'INVOICE' && state.invoiceDate) {
      displayDate = state.invoiceDate; // Use live state if creating invoice from scratch
  }
  ...
  // Use record ID/InvoiceNum OR state InvoiceNum
  const metaValue = (type === 'INVOICE' || type === 'RECEIPT') 
    ? (record?.invoiceNumber || state.invoiceNumber || "DRAFT")
    : (record?.id.substring(0, 8).toUpperCase() || "NEW");
  ...
  drawCustomerBox(doc, customer, yPos, type === 'ESTIMATE' ? "Estimate For:" : "Bill To:", {
    ...
    terms: type === 'INVOICE' ? (record?.paymentTerms || state.paymentTerms || 'Due on Receipt') : undefined
  });
  ...
  const mode = record ? record.inputs.mode : state.mode; 
  ...
  if (pricingMode === 'level_pricing' && results.laborCost > 0) {
      const hrs = record ? (record.expenses.manHours) : state.expenses.manHours;
      tableRows.push([
          'Labor',
          `Application Labor (${hrs} hours)`,
          '-',
          state.showPricing ? formatCurrency(results.laborCost) : '-'
      ]);
  }

  // Expenses - use record or state
  const expenses = record ? record.expenses : state.expenses;

  if (expenses.tripCharge > 0) tableRows.push(['Trip Charge', 'Standard Rate', '1', state.showPricing ? formatCurrency(expenses.tripCharge) : '-']);
  if (expenses.fuelSurcharge > 0) tableRows.push(['Fuel Surcharge', 'Distance Adjustment', '1', state.showPricing ? formatCurrency(expenses.fuelSurcharge) : '-']);
  if (expenses.other && expenses.other.amount !== 0) tableRows.push([expenses.other.description || 'Adjustment', 'Misc', '1', state.showPricing ? formatCurrency(expenses.other.amount) : '-']);
